...
export function applyFilters(notifications: GitNotification[], rules: FilterRule[]): GitNotification[] {
    return notifications
        .filter(notification => rules.length == 0 || rules.some(rule => rule.kind === "INCLUDE" && rule.reason === notification.reason))
        .filter(notification => !rules.some(rule => rule.kind === "EXCLUDE" && rule.reason === notification.reason))
}
